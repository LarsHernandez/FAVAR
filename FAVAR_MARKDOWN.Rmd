...
infl.arima <- forecast(m, h = 12)

m <- auto.arima(dat$PROD)
prod.arima <- forecast(m, h = 12)

m <- auto.arima(dat$FFR)
ffr.arima <- forecast(m, h = 12)


arima <- tibble(Date = actual$Date,
                FFR = ffr.arima$mean,
                INFL = infl.arima$mean,
                PROD = prod.arima$mean, 
                TYPE = "ARIMA")
...
RMSE(infl.arima$mean, actual$INFL)

RMSE(pred$fcst$FFR[,1], actual$FFR)
RMSE(ffr.arima$mean, actual$FFR)

RMSE(pred$fcst$PROD[,1], actual$PROD)
RMSE(prod.arima$mean, actual$PROD)
```
...
for (i in 1:n.columns) {
  result <- pp %>% 
    ...
    dplyr::select(-Date, -type) %>% VAR(p = 2)
