...
tibble(Date = FFR$Date, FFR = FFR$Value, INFL = CPI$Value, PROD = PROD$Value) %>% 
  ...
  labs(title=expression(paste(bold("Figur 4.1  "), "Variabler til simpel rekursiv VAR")), 
       subtitle="Note: Måneds data indhentet gennem Quandl 2.9.1, transformationerne er:\nINFL 1. logdifferens, PROD 1. logdifferens, FFR 1. differens", 
...
df <- tibble(R1 = irf(VAR(pp[c(1,2,3,4)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             R2 = irf(VAR(pp[c(1,2,4,3)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             R3 = irf(VAR(pp[c(1,3,2,4)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             R4 = irf(VAR(pp[c(1,3,4,2)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             R5 = irf(VAR(pp[c(1,4,2,3)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             R6 = irf(VAR(pp[c(1,4,3,2)][,c(2:4)], p = 13), impulse = "FFR", response = "INFL", n.ahead = 24)$irf$FFR,
             N  = c(0:24))
...
df %>% gather(variable, Change, -N) %>% 
  ...
  labs(title = expression(paste(bold("Figure 4.2  "), "Impulse response from FFR to INFL")), 
       subtitle = "All 6 orderings in the reduced form VAR variance covariance matrix", 
       color = "Order", y = "", x = "Lags", caption = "Kilde: FRED + Egne beregninger") +
  th
...
df %>% ggplot(aes(N, value, linetype = variable)) +
  ...
  geom_hline(aes(yintercept = 0), size = 0.5, alpha = 0.5) +
  scale_y_continuous(breaks = c(0.10,0.05,0,-0.05,-0.10), limits = c(-0.12, 0.12)) +
...
rbind(actual, prediction, arima) %>% 
  ...
  geom_line() +
  facet_wrap(~variable) + 
  labs(title = expression(paste(bold("Figure 4.5  "), "Forecast error")), 
       subtitle = "VAR(13) compared against ARIMA(2,0,1)", 
       linetype = "Model", y = "", x = "", caption = "Kilde: FRED + Egne beregninger") +
  th
